...
namespace Menu
{
	std::string m_sMenuName = "gamereverse - reverse your senses";
...
void Menu::Render( )
{
	...
	if ( LoadSave )
	{
		...
		{
			for ( int i = 0; i < 20; i++ )
				PopUp.push_back( { "you are homo",  Color( 255.f,0.f,0.f,255.f ), Interfaces::GlobalVars->realtime + 30.f } );
	...
	Frame( m_sMenuName );
	{
		...
		Tab( "antiaim" );
		{
			SubTab( "main" );
			{
				...
				//CheckBox("New AntiAim", Config.newantiaim);
			}
			/* SubTab( "left" );
			{
		...
		Tab( "misc" );
		{
			...
			//TextBox();
			//CheckBox("clantag", Config.clantag);
			/*SubTab("config"); {
				ComboBox("config name", { "hvh", "hvh2", "legit" }, Config.confignam);
				CheckBox("load config", Config.loadconfig); {
					LoadConfig();
				}
				CheckBox("save config", Config.saveconfig); {
					SaveConfig();
				}
			}*/
		}
...
void Menu::Frame( std::string sName ) 
{
	...
	Draw::String( m_uPos.x + 6, m_uPos.y + ( m_uSize.y - 18 ), Draw::Font.Default, Color( 255, 255, 255, 255 ), "gamereverse.paste version", false );
