...
void Rage::AntiAim( CUserCmd* pCmd )
{
	...
	if (Menu::Config.newantiaim) {
/*		int LocalPlayer = Interfaces::Engine->GetLocalPlayer;
		float Desync = LocalPlayer->DesyncValue() / 100 * Math::NormalizeAngles(pCmd->viewangles.y) + 30.0f;
		float MM = 1.10f;
		...
		if (g_pGlobalVars->curtime < LBY && pCmd->sidemove < 4 && pCmd->sidemove > -4)
			pCmd->sidemove = MM;

		if (!Globals::bSendPacket)
		{
			if (g_pGlobalVars->curtime >= LBY && LocalPlayer->GetVelocity().Length() < 4)
			{
